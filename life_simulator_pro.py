...
<style>
  ...
  section.main, .main, [data-testid="stAppViewContainer"] { overscroll-behavior: contain !important; }
  * { -webkit-tap-highlight-color: transparent; }
  div[data-testid="stTabs"] button[role="tab"] {
    font-size:18px !important; font-weight:700 !important; padding:14px 36px !important;
    border-radius:10px 10px 0 0 !important; color:#555 !important;
    background:#f0f2f6 !important; border:2px solid #ddd !important;
    border-bottom:none !important; margin-right:6px !important; transition:all 0.2s !important;
  }
  div[data-testid="stTabs"] button[role="tab"]:hover { background:#e0e8ff !important; color:#1a6aff !important; }
  div[data-testid="stTabs"] button[role="tab"][aria-selected="true"] {
    background:#1a6aff !important; color:#ffffff !important; border-color:#1a6aff !important;
    font-size:19px !important; box-shadow:0 4px 12px rgba(26,106,255,0.3) !important;
  }
  div[data-testid="stTabs"] [data-testid="stTabBar"] {
    gap:4px !important; border-bottom:3px solid #1a6aff !important;
    padding-bottom:0 !important; margin-bottom:16px !important;
  }
</style>
...
def linked_int(label, lo, hi, default, step, key, disabled=False, man=False):
    """整数スライダー + 数値入力ボックス（man=True で万円単位）"""
    vk     = "val_" + key
    sl_key = "wsl_" + key
    nb_key = "wnb_" + key
    ...
    if man:
        ...
        # スライダーとnbを現在値に同期
        st.session_state[sl_key] = cur_m
        st.session_state[nb_key] = cur_m

        def _sl_change(vk=vk, sl_key=sl_key):
            st.session_state[vk] = int(st.session_state[sl_key]) * 10_000

        def _nb_change(vk=vk, nb_key=nb_key, lo_m=lo_m, hi_m=hi_m):
            raw = st.session_state.get(nb_key, 0)
            st.session_state[vk] = int(clamp(raw, lo_m, hi_m)) * 10_000

        col_sl, col_nb = st.columns([4, 1])
        with col_sl:
            st.slider(f"{label}（万円）", min_value=lo_m, max_value=hi_m,
                      value=cur_m, step=step_m, disabled=disabled,
                      key=sl_key, format="%d 万円", on_change=_sl_change)
        with col_nb:
            st.number_input("万円", min_value=lo_m, max_value=hi_m,
                            value=cur_m, step=step_m, disabled=disabled,
        ...
        def _sl_change(vk=vk, sl_key=sl_key):
            st.session_state[vk] = int(st.session_state[sl_key])

        def _nb_change(vk=vk, nb_key=nb_key, lo=lo, hi=hi):
            raw = st.session_state.get(nb_key, 0)
            st.session_state[vk] = int(clamp(raw, lo, hi))

        col_sl, col_nb = st.columns([4, 1])
        with col_sl:
            st.slider(label, min_value=int(lo), max_value=int(hi),
                      value=cur, step=int(step), disabled=disabled,
                      key=sl_key, on_change=_sl_change)
        with col_nb:
            st.number_input("値", min_value=int(lo), max_value=int(hi),
                            value=cur, step=int(step), disabled=disabled,
                            key=nb_key, on_change=_nb_change,
                            label_visibility="collapsed")
        return int(st.session_state[vk])
...
with tab_input:
    ...
    with st.expander("NISA 詳細設定", expanded=nisa_on):
        ...
        st.caption("⚠ 新NISAのつみたて投資枠の上限は月10万円（年120万円）、成長投資枠含む年間上限は360万円（月30万円）です。本シミュレーターでは試算用に最大100万円/月まで入力可能です。")
    ...
    for i in range(1, 13):
        ...
        events.append({"on": bool(on), "label": label, "idx": i,
                        "age": int(ev_age), "direction": direction, "amount": int(amount)})
...
st.dataframe(_df_confirm, use_container_width=True, hide_index=True,
...
if run_clicked:
    with st.spinner("⏳ シミュレーション計算中..."):
        ...
        # ── key_events: elabel は ASCII のみ（文字化け回避） ──
        key_events = [
            {"age": params["retire_age"],
             "label":  f"退職（{params['retire_age']}歳）",
             "elabel": f"Retire (age {params['retire_age']})",
             "color": "#e67e22"},
            {"age": params["pension_start_age"],
             "label":  f"年金開始（{params['pension_start_age']}歳）",
             "elabel": f"Pension (age {params['pension_start_age']})",
             "color": "#2980b9"},
        ]
        for ev in params["events"]:
            if ev["on"]:
                sign = "+" if ev["direction"] == "収入" else "-"
                # elabel は番号と金額のみ（日本語ラベルを除外して文字化け回避）
                key_events.append({
                    "age":    ev["age"],
                    "label":  f"[Ev{ev['idx']}] {ev['label']}（{sign}{ev['amount']//10000:,}万円）",
                    "elabel": f"Ev{ev['idx']} {sign}{ev['amount']//10000:,}M (age {ev['age']})",
        ...
        if ruin_thr_age:
            key_events.append({
                ...
                "elabel": f"Ruin>{params['ruin_threshold']}% (age {ruin_thr_age})",
                "color":  "#8e44ad",
            })
...
if st.session_state.sim_done and st.session_state.sim_result is not None:
    ...
    <div style="text-align:center;margin:8px 0;">
      <button onclick="(function(){var t=window.parent.document.querySelectorAll('div[data-testid=stTabs] button[role=tab]');if(t.length>=2){t[1].click();}})()"
        style="font-size:18px;font-weight:700;color:#fff;background:#1a6aff;border:none;
               border-radius:10px;padding:14px 40px;cursor:pointer;
               box-shadow:0 4px 12px rgba(26,106,255,0.35);">
        📈 グラフ・結果を見る →
      </button>
      <div style="margin-top:10px;font-size:14px;color:#888;">※ 上にスクロールしてご確認ください。</div>
    </div>""", height=100)
...
with tab_result:
    ...
    for ke in sorted(key_events, key=lambda x: x["age"]):
        ...
        ax.annotate(ke["elabel"],          # ← ASCII only
    ...
    if ruin_thr_age is not None:
        ...
        ax2.text(ruin_thr_age + 0.3, 88, f"Ruin>{threshold}% at age {ruin_thr_age}",
...
<tr><th style="text-align:left;padding:3px 8px;color:#555;border-bottom:1px solid #dde3ff;">English (graph)</th>
    <th style="text-align:left;padding:3px 8px;color:#555;border-bottom:1px solid #dde3ff;">日本語</th>
    <th style="text-align:left;padding:3px 8px;color:#555;border-bottom:1px solid #dde3ff;">備考</th></tr>
<tr><td><span style="color:#4c72b0">■</span> Total Assets (10-90%)</td><td><b>総資産の10〜90%帯</b></td><td>モンテカルロ分布範囲</td></tr>
<tr><td><span style="color:#1a6aff">─</span> Total (avg)</td><td><b>総資産（平均）</b></td><td>全口座合計</td></tr>
...
<tr><td><span style="color:#c0392b">─</span> Ruin Probability</td><td><b>破綻確率（累積）</b></td><td>総資産≤0 の割合</td></tr>
<tr><td><span style="color:#8e44ad">- -</span> Threshold</td><td><b>警告しきい値</b></td><td>設定した破綻確率の閾値</td></tr>
</table>
"""

    # アノテーション（グラフ内吹き出し）の日本語対比
    ev_rows = ""
    for ke in sorted(key_events, key=lambda x: x["age"]):
        color = ke["color"]
        ev_rows += (f'<tr><td style="color:{color};font-weight:700;">{ke["elabel"]}</td>'
                    f'<td>{ke["label"]}</td></tr>\n')

    legend_lines_html += f"""
<br><b>📌 グラフ内アノテーション対比表</b><br>
<table style="border-collapse:collapse;width:100%;font-size:13px;">
<tr><th style="text-align:left;padding:3px 8px;color:#555;border-bottom:1px solid #dde3ff;">Graph label (English)</th>
    <th style="text-align:left;padding:3px 8px;color:#555;border-bottom:1px solid #dde3ff;">日本語</th></tr>
{ev_rows}</table>
</div>"""

    st.markdown(legend_lines_html, unsafe_allow_html=True)
